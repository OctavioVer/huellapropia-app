...
function ImpactBadge({ type, text }: { type: string; text: string }) {
  const colors: any = {
    ...
    neutral: {
      bg: "rgba(203,166,82,0.12)",
      border: "rgba(203,166,82,0.3)",
      text: "#CBA652",
      icon: "◆",
    },
  };
...
export default function AnalisisPage() {
  ...
  const getAuthHeader = (): Record<string, string> => {
    ...
    if (!raw) return {};
    try {
      return { Authorization: `Bearer ${JSON.parse(raw).token}` };
    } catch {
      return {};
    }
  };
...

{
    ...
    try {
      ...
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "user",
              content: `Analiza el siguiente proyecto de ley y genera el informe ejecutivo en JSON:\n\n${billText}`,
            },
          ],
        }),
      });
      ...
      setError(
        err.message.includes("JSON")
          ? "Error procesando la respuesta. Intentá nuevamente."
          : `Error: ${err.message}`,
      );
    } finally {
      setLoading(false);
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      const W = 210,
        ...
        CW = W - M * 2;
      let y = 0;
      const GOLD: [number, number, number] = [203, 166, 82],
        DARK_BG: [number, number, number] = [15, 20, 35],
        HEADER_BG: [number, number, number] = [20, 28, 50],
        WHITE: [number, number, number] = [240, 244, 248],
        LGRAY: [number, number, number] = [160, 174, 192],
        GREEN: [number, number, number] = [74, 222, 128],
      ...
      const writeBullets = (items: string[], opts: any = {}) => {
        const { marker = "•" } = opts;
        (items || []).forEach((item, i) => {
          const prefix = marker === "num" ? `${i + 1}.` : marker;
          checkPage(6);
          doc.setFont("helvetica", "bold");
          doc.setFontSize(9.5);
      ...
      y += 5;
      const urgCol: [number, number, number] =
        analysis.urgencia === "Alta"
          ? (RED as [number, number, number])
      ...
      writeText(analysis.resumen_ejecutivo);
      sectionTitle("Puntos Clave");
      writeBullets(analysis.puntos_clave, { marker: "num", markerColor: GOLD });
      ...
      y += 5;
      writeBullets(analysis.impactos_positivos, {
        marker: "+",
        markerColor: GREEN,
      });
      doc.setFont("helvetica", "bold");
      ...
      y += 5;
      writeBullets(analysis.impactos_negativos, {
        marker: "-",
        markerColor: RED,
      });
      sectionTitle("Sectores Afectados");
      writeBullets(analysis.sectores_afectados, {
        marker: "◆",
        markerColor: GOLD,
      });
      sectionTitle("Impacto Fiscal");
      writeText(analysis.impacto_fiscal);
      sectionTitle("Antecedentes Legislativos");
      writeText(analysis.antecedentes);
      sectionTitle("Recomendacion de Voto");
      checkPage(16);
      const vCol = (analysis.recomendacion_voto || "").includes("Acompa")
        ? GREEN
      ...
      y += 5;
      writeBullets(analysis.argumentos_a_favor, {
        marker: "+",
        markerColor: GREEN,
      });
      doc.setFont("helvetica", "bold");
      ...
      y += 5;
      writeBullets(analysis.argumentos_en_contra, {
        marker: "-",
        markerColor: RED,
      });
      sectionTitle("Modificaciones Sugeridas");
      writeBullets(analysis.modificaciones_sugeridas, {
        marker: "→",
        markerColor: GOLD,
      });
      ...
      const safeName = (analysis.titulo || "Informe")
  ...
  const urgencyColor = (u: string) =>
    u === "Alta" ? "#EF4444" : u === "Media" ? "#F59E0B" : "#22C55E";
    ...
    <div style={{ minHeight: "100vh" }}>
      ...
      <div
        style={{
          ...
          borderBottom: "1px solid rgba(203,166,82,0.12)",
          padding: "12px 24px",
          display: "flex",
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <Link
            href="/historial"
            style={{
              background: "rgba(203,166,82,0.08)",
              border: "1px solid rgba(203,166,82,0.2)",
              color: "#CBA652",
              fontSize: "12px",
              padding: "6px 14px",
              borderRadius: "8px",
      ...
      <div
        style={{ maxWidth: "880px", margin: "0 auto", padding: "36px 20px" }}
      >
        <div style={{ textAlign: "center", marginBottom: "40px" }}>
          ...
          <p style={{ color: "#64748B", fontSize: "15px" }}>
            Pegá el texto de cualquier proyecto y obtené un informe ejecutivo en
            segundos.
          </p>
        </div>
          ...
          >
            ...
            >
              ...
              <button
                ...
                style={{
                  background: "rgba(203,166,82,0.1)",
                  border: "1px solid rgba(203,166,82,0.25)",
                  color: "#CBA652",
                  fontSize: "12px",
                  padding: "6px 14px",
                  borderRadius: "8px",
            ...
            <textarea
              ...
              placeholder="Pegá aquí el texto completo del proyecto de ley..."
              style={{
                width: "100%",
                minHeight: "220px",
            ...
            >
              <span style={{ fontSize: "12px", color: "#475569" }}>
                {billText.length > 0
                  ? `${billText.split(/\s+/).filter(Boolean).length} palabras`
                  : ""}
              </span>
              <button
                ...
                style={{
                  background:
                    loading || !billText.trim()
                      ? "rgba(100,116,139,0.2)"
                      : "linear-gradient(135deg, #CBA652, #A88B3A)",
                  color: loading || !billText.trim() ? "#475569" : "#0B1120",
                  border: "none",
                  borderRadius: "10px",
                  padding: "14px 32px",
                  fontSize: "14px",
                  fontWeight: 700,
                  cursor:
                    loading || !billText.trim() ? "not-allowed" : "pointer",
                  boxShadow:
                    loading || !billText.trim()
                      ? "none"
          ...
          >
            <div
              style={{
                width: "48px",
                height: "48px",
                border: "3px solid rgba(203,166,82,0.15)",
                borderTop: "3px solid #CBA652",
                borderRadius: "50%",
                margin: "0 auto 20px",
          ...
          <div>
            ...
            >
              ...
              >
                <span
                  style={{
                    background: `rgba(${analysis.urgencia === "Alta" ? "239,68,68" : analysis.urgencia === "Media" ? "245,158,11" : "34,197,94"},0.12)`,
                    color: urgencyColor(analysis.urgencia),
                    border: `1px solid ${urgencyColor(analysis.urgencia)}40`,
                    padding: "4px 12px",
                    borderRadius: "6px",
                    fontSize: "12px",
                    fontWeight: 600,
                ...
                <button
                  ...
                  style={{
                    ...
                    boxShadow: exporting
                      ? "none"
                      : "0 2px 12px rgba(203,166,82,0.3)",
                  }}
                >
            ...
            <AnalysisSection icon="📋" title="Proyecto" delay={0}>
              ...
              >
                <span>📄 {analysis.tipo}</span>
                <span>🏛️ {analysis.comision}</span>
              </div>
            </AnalysisSection>

            <AnalysisSection icon="📝" title="Resumen Ejecutivo" delay={100}>
              <p style={{ margin: 0 }}>{analysis.resumen_ejecutivo}</p>
            </AnalysisSection>
            ...
            <AnalysisSection icon="⚡" title="Análisis de Impacto" delay={300}>
              <div style={{ marginBottom: "12px" }}>
                ...
                >
                  {analysis.impactos_positivos?.map(
                    (imp: string, i: number) => (
                      <ImpactBadge key={i} type="positive" text={imp} />
                    ),
                  )}
                </div>
              </div>
              <div style={{ marginTop: "16px" }}>
                ...
                >
                  {analysis.impactos_negativos?.map(
                    (imp: string, i: number) => (
                      <ImpactBadge key={i} type="negative" text={imp} />
                    ),
                  )}
                </div>
              </div>
            ...
            <AnalysisSection icon="🏢" title="Sectores Afectados" delay={350}>
              <div style={{ display: "flex", flexWrap: "wrap" }}>
                {analysis.sectores_afectados?.map((s: string, i: number) => (
                  <ImpactBadge key={i} type="neutral" text={s} />
                ))}
              </div>
            </AnalysisSection>

            <AnalysisSection icon="💰" title="Impacto Fiscal" delay={400}>
              <p style={{ margin: 0 }}>{analysis.impacto_fiscal}</p>
            </AnalysisSection>
            <AnalysisSection
              icon="📚"
              title="Antecedentes Legislativos"
              delay={450}
            >
              <p style={{ margin: 0 }}>{analysis.antecedentes}</p>
            </AnalysisSection>
            ...
            >
              ...
              >
                <span
                  style={{
                    background:
                      analysis.recomendacion_voto === "Acompañar"
                        ? "rgba(34,197,94,0.15)"
                        : analysis.recomendacion_voto === "Rechazar"
                          ? "rgba(239,68,68,0.15)"
                    ...
                    fontWeight: 700,
                    fontFamily: "'Source Serif 4', Georgia, serif",
                    border: `1px solid ${analysis.recomendacion_voto === "Acompañar" ? "rgba(34,197,94,0.3)" : analysis.recomendacion_voto === "Rechazar" ? "rgba(239,68,68,0.3)" : "rgba(245,158,11,0.3)"}`,
                  }}
                >
                  {analysis.recomendacion_voto}
                </span>
              </div>
              ...
              >
                <div>
                  ...
                  {analysis.argumentos_a_favor?.map((a: string, i: number) => (
                    <p
                      key={i}
                      style={{
                        fontSize: "13px",
                ...
                <div>
                  ...
                  {analysis.argumentos_en_contra?.map(
                    (a: string, i: number) => (
                      <p
                        key={i}
                        style={{
            ...
            <AnalysisSection
              ...
              delay={550}
            >
              {analysis.modificaciones_sugeridas?.map(
                (m: string, i: number) => (
                  <div
                    key={i}
                    style={{
            ...
            >
              <p style={{ margin: 0 }}>
                Informe generado por IA — Huella Propia · Análisis Legislativo
                Inteligente
              </p>
              <p style={{ margin: "4px 0 0 0", fontSize: "11px" }}>
                Este análisis es orientativo. Siempre validar con el equipo
                legal del bloque.
              </p>
            </div>
          </div>
